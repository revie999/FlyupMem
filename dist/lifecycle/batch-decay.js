 ...
 */
export function batchDecay(store) {
    const now = new Date();
    ...
    for (const eng of store.engrams) {
        if (eng.status === 'locked' || eng.status === 'retired')
            continue;
        const lastAccess = new Date(eng.activation.last_accessed);
        const daysSince = (now.getTime() - lastAccess.getTime()) / (1000 * 60 * 60 * 24);
        if (daysSince < 0.5)
            continue; // Skip if accessed today
        const newStrength = decayedStrength(eng.activation.retrieval_strength, daysSince, 'raw', eng.emotional_weight ?? 5);
        const oldStatus = eng.status;
        const newStatus = statusFromStrength(newStrength);
    ...
    for (const obs of store.observations) {
        if (obs.status === 'locked' || obs.status === 'retired')
            continue;
        const lastAccess = new Date(obs.activation.last_accessed);
        const daysSince = (now.getTime() - lastAccess.getTime()) / (1000 * 60 * 60 * 24);
        if (daysSince < 0.5)
            continue;
        const newStrength = decayedStrength(obs.activation.retrieval_strength, daysSince, 'observation', obs.emotional_weight ?? 5);
    ...
    for (const mm of store.mentalModels) {
        if (mm.status === 'locked' || mm.status === 'retired')
            continue;
        const lastAccess = new Date(mm.activation.last_accessed);
        const daysSince = (now.getTime() - lastAccess.getTime()) / (1000 * 60 * 60 * 24);
        if (daysSince < 0.5)
            continue;
        const newStrength = decayedStrength(mm.activation.retrieval_strength, daysSince, 'mental_model', mm.emotional_weight ?? 5);
...
//# sourceMappingURL=batch-decay.js.map