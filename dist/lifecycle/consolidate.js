 ...
 */
export function hasPolarityConflict(cluster) {
 ...
 */
export function mergeToObservation(cluster) {
    ...
    // Collect evidence
    const evidence = cluster.map(eng => ({
        engram_id: eng.id,
    ...
    return {
        ...
        evidence,
        trend: 'new',
        confidence: Math.min(10, Math.round(cluster.reduce((s, e) => s + (e.confidence ?? 5), 0) / cluster.length + 1)),
        activation: {
 ...
 */
export function markAsEvolution(cluster, store) {
    for (const eng of cluster) {
 ...
 */
export async function consolidateUnmerged(store, batchSize = 50) {
