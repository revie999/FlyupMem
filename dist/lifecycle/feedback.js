 ...
 */
export function applyFeedback(memoryId, signal, store, context) {
    const mem = store.getById(memoryId);
    if (!mem)
        return { applied: false, retired: false, reason: 'memory not found' };
    ...
    if ('feedback' in mem && mem.feedback) {
        if (signal === 'positive')
            mem.feedback.positive++;
        else if (signal === 'negative')
            mem.feedback.negative++;
        else
            mem.feedback.neutral++;
    }
    ...
    if (mem.layer === 'raw') {
        store.updateEngram(memoryId, mem);
    }
    store.save();
 ...
 */
export function getFeedbackSummary(memoryId, store) {
    const mem = store.getById(memoryId);
    if (!mem)
        return null;
    ...
    return {
        memoryId,
        feedback: 'feedback' in mem ? mem.feedback : { positive: 0, negative: 0, neutral: 0 },
        recentFeedback,
