 ...
 */
export function decayedStrength(retrievalStrength, daysSinceAccess, layer, emotionalWeight = 5) {
 ...
 */
export function reactivate(current) {
    return Math.min(1.0, current + 0.1);
}
/**
 * Map retrieval strength to status.
 */
export function statusFromStrength(strength) {
    if (strength > 0.5)
        return 'active';
    if (strength > 0.3)
        return 'fading';
 ...
 */
export function computeActivation(activation, layer, emotionalWeight = 5) {
    const now = Date.now();
